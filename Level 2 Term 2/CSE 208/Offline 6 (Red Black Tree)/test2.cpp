...
struct RBT
{
    ...
    {
        Node *temp = node->child[right];
        node->child[right] = temp->child[left];
    ...
    {
        Node *temp = node->child[left];
        node->child[left] = temp->child[right];
        temp->child[right] = node;
    ...
    Node* DELETE_FIX_UP(Node *node,bool dir, bool &ok)
    {
        ...
        if(sibling != NULL)
        {
            ...
            else /// Case 2 Part 2 : Black Sibling with not all black children
            {
                ...
                {
                    if(dir==left)
                        parent = rotateLeft(parent); /// single rotation
                    else
                        parent = rotateRight(parent); /// single rotation
                }
                else
                {
                    if(dir==left)
                        parent = doubleRotateLeft(parent); /// align and rotate
                    else
                        parent = doubleRotateRight(parent); /// align and rotate
                }
