...
struct RBT
{
    ...
    Node* INSERT_FIX_UP(Node* node,bool dir)
    {
        if(red(node->child[dir]))
        {
            if(red(node->child[!dir]))
            {
                if(red(node->child[dir]->child[dir]) || red(node->child[dir]->child[!dir])) colorFlip(node);
            }
            else
            {
                if(red(node->child[dir]->child[dir])) node = rotate(node,!dir);
                else if(red(node->child[dir]->child[!dir])) node = doubleRotate(node,!dir);
            }
        }
        return node;
    }
    ...
    Node* __delete(Node *node,int data,bool &ok)
    {
        ...
        else
        {
            ...
            if(ok==false) node = DELETE_FIX_UP(node,dir,ok);
        }
        return node;
    }
    ...
    Node* DELETE_FIX_UP(Node *node,bool dir, bool &ok)
    {
        ...
        if(sibling != NULL)
        {
            ...
            else /// Case 2 Part 2 : Black Sibling with not all black children
            {
                ...
                if(isRedSiblingReduction) node->child[dir] = parent; /// fixing the child for proper bottom up fixing later
                else node = parent; /// usual black case
