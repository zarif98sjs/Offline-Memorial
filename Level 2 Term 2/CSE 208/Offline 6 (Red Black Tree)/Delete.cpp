...
//    }
    ...
    {
        if(red(node->left)) node = rightRotate(node);
    ...
    Node* __delete(Node *node,int data)
    {
        ...
        if(data < node->data)
        {
            if(red(node->right)) node = leftRotate(node);

            if(!red(node->left) && !red(node->left->right)) node = alignAndShiftRedLeft(node);
            if(!red(node->left) && !red(node->left->left)) node = shiftRedLeft(node);
...
//                node->left = __delete(node->left,node->data);
                ...
                node->data = temp->data;
                node->left = __delete(node->left,temp->data);
            }
        }
