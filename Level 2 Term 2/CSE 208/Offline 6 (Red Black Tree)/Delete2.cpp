...
struct RBT
{
    ...
    Node* alignAndShiftRedLeft(Node *node)
    {
        colorFlip(node);
    ...
    Node* alignAndShiftRedRight(Node *node)
    {
        colorFlip(node);
...
//        if(!red(node->right) && !red(node->right->left)) node = alignAndShiftRedRight(node);
//        if(!red(node->right) && !red(node->right->right)) node = shiftRedRight(node);
...
//            if(!red(node->left) && !red(node->left->right)) node = alignAndShiftRedLeft(node);
            if(!red(node->left) && !red(node->left->left))
            {
                if(red(node->right->left)) node = alignAndShiftRedLeft(node);
                if(red(node->right->right)) node = shiftRedLeft(node);
            }
...
//            if(!red(node->right) && !red(node->right->left)) node = alignAndShiftRedRight(node);
            ...
            {
                if(red(node->left->right)) node = alignAndShiftRedRight(node);
                if(red(node->left->left)) node = shiftRedRight(node);
            }
