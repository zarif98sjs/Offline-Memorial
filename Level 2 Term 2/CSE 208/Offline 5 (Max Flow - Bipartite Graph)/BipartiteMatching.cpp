...
public:
    ...
    int dfs(int u, int amount,vector<int>&bmatch)
    {
        ...
        for(int v:g.adjList[u]) /// another way would be to make the adj list undirected and traverse over that
        {
            if(!vis[v] && cap[u][v]>0)
            {
                int sent = dfs(v,min(amount,cap[u][v]),bmatch);

                if(sent > 0) /// if any path with bottleneck > 0 exists
                {
//                    cout<<u<<" , "<<v<<endl; /// the path
                    if(u!=source && u!=sink) bmatch.push_back(u);
                    if(v!=source && v!=sink) bmatch.push_back(v);
    ...
    int max_flow(int source,int sink, vector<int>&bmatch)
    {
        this->source = source;
        this->sink = sink;
        ...
        while(sent != 0)
        {
            ...
            sent = dfs(source,INT_MAX,bmatch);
//            cout<<"Done"<<endl;
            MAXFLOW += sent;
        }
    ...
    void print_edges()
    {
        ...
        {
            int u = e.src , v = e.dst;
            cout<<u<<" "<<v<<" "<<init[u][v] - cap[u][v]<<"/"<<init[u][v]<<endl;
        }
    }
};
...
//        cout<<src<<" , "<<dst<<endl;
        ...
        vector<int>bmatch;
        int max_match = f.max_flow(src,dst,bmatch);

        cout<<max_match<<endl;

        reverse(bmatch.begin(),bmatch.end());

        for(int i=0;i<(int)bmatch.size();i+=4)
            cout<<bmatch[i]<<" "<<bmatch[i+1]<<endl;
    }
